                ...
                <div className="member-progress-list">
                  ...
                  {group.members.map(member => {
                    const tasks = member.assignedTasks || [
                      { taskName: `Nhiệm vụ 1 (Giao bởi nhóm trưởng)`, status: 'done' },
                      { taskName: `Nhiệm vụ 2 (Phần backend)`, status: member.tasksCompleted > 1 ? 'done' : 'inProgress' },
                      { taskName: `Nhiệm vụ 3 (Phần frontend)`, status: member.tasksCompleted > 2 ? 'done' : 'pending' }
                    ];
                    return (
                    <div className="member-progress-item" key={member.userId} style={{ marginBottom: '15px' }}>
                      ...
                      {/* Dropdown Task */}
                      <details style={{ marginTop: '8px', fontSize: '13px', background: '#f8fafc', padding: '5px 10px', borderRadius: '4px', border: '1px solid #e2e8f0', cursor: 'pointer' }}>
                        <summary style={{ outline: 'none', color: '#475569', fontWeight: '500' }}>Xem chi tiết nhiệm vụ</summary>
                        <ul style={{ listStyle: 'none', padding: 0, margin: '8px 0 0 0' }}>
                          {tasks.map((t, idx) => (
                            <li key={idx} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px solid #f1f5f9' }}>
                              <span style={{ color: '#334155' }}>{t.taskName}</span>
                              <span className={`status-badge ${t.status}`} style={{ fontSize: '11px', padding: '2px 6px', borderRadius: '4px', background: t.status === 'done' ? '#dcfce7' : t.status === 'inProgress' ? '#dbeafe' : '#fef3c7', color: t.status === 'done' ? '#166534' : t.status === 'inProgress' ? '#1e40af' : '#92400e' }}>
                                {t.status === 'done' ? 'Hoàn thành' : t.status === 'inProgress' ? 'Đang làm' : 'Chờ làm'}
                              </span>
                            </li>
                          ))}
                        </ul>
              ...
              <table className="evaluation-table">
                ...
                <tbody>
                  {evalGroup.members.map(member => {
                    ...
                    const groupTotalTasks = evalGroup.tasks.done + evalGroup.tasks.inProgress + evalGroup.tasks.overdue + evalGroup.tasks.pending;
                    const completedTasksList = Array.from({ length: member.tasksCompleted }).map((_, i) => `Task đã hoàn thành ${i+1}`);
                    return (
                      <tr key={member.userId}>
                        <td><strong>{member.fullName}</strong></td>
                        <td>{member.role === 'leader' ? '👑 Nhóm trưởng' : 'Thành viên'}</td>
                        <td style={{ textAlign: 'center' }}>
                          <div style={{ fontWeight: 'bold', color: '#2563eb' }}>{member.tasksCompleted} / {groupTotalTasks}</div>
                          {member.tasksCompleted > 0 && (
                            <details style={{ marginTop: '5px', fontSize: '12px', textAlign: 'left', cursor: 'pointer' }}>
                              <summary style={{ color: '#64748b' }}>Các task hoàn thành</summary>
                              <ul style={{ paddingLeft: '15px', marginTop: '5px', color: '#334155' }}>
                                {completedTasksList.map((t, idx) => <li key={idx}>{t}</li>)}
                              </ul>
                            </details>
                          )}
                        </td>
